...
const FAQS: Record<string, Array<{ question: string; answer: string }>> = {
  ...
  'kak-izbavitsya-ot-trevogi': [
    { question: 'Как быстро убрать тревогу прямо сейчас?', answer: 'На сессии я использую дыхательные техники и мягкое трансовое воздействие, чтобы снизить уровень тревоги за 5–10 минут. Вы получаете конкретные техники самопомощи, которые работают даже в острых состояниях.' },
    { question: 'Можно ли избавиться от тревоги навсегда?', answer: 'Да, если работать не с симптомами, а с причиной тревоги на уровне подсознания. Гипнотерапия помогает перезаписать те нейронные связи, которые держат вас в тревожном состоянии. Это не временное облегчение — это глубокая перестройка реакции нервной системы.' },
    { question: 'Тревога — это лечится или это на всю жизнь?', answer: 'Тревога прекрасно поддаётся коррекции. Это не приговор и не особенность характера. Тревожность — это паттерн реакции нервной системы, который сформировался как защитный механизм. С ним можно работать, и гипнотерапия даёт устойчивый результат.' },
    { question: 'Что делать, если тревога мешает работать и жить?', answer: 'Первый шаг — прекратить бороться с тревогой и начать понимать, откуда она берётся. На бесплатной консультации мы разберём ваш случай, я покажу, как работает гипнотерапия, и вы почувствуете облегчение уже на первой встрече.' },
    { question: 'Помогает ли гипноз от тревоги?', answer: 'Да, это один из самых эффективных методов работы с тревожностью. Гипноз позволяет обойти сознательное сопротивление и обратиться напрямую к подсознанию, где хранятся истинные причины тревоги. Результат — спокойствие без постоянного самоконтроля.' },
  ],
  'postoyannaya-trevoga-bez-prichiny': [
    { question: 'Почему я тревожусь без причины?', answer: 'Причина есть всегда — просто она находится не на поверхности, а в подсознании. Это может быть старый травматический опыт, подавленная эмоция или усвоенный в детстве паттерн реагирования. Мозг «запомнил» опасность и продолжает сигналить, даже когда угрозы нет.' },
    { question: 'Это нормально — чувствовать тревогу, когда всё хорошо?', answer: 'Нет, это не нормально, но это объяснимо. Если внешне всё стабильно, а внутри тревога — значит, источник не во внешних обстоятельствах, а во внутреннем состоянии. Так проявляются неразрешённые конфликты подсознания.' },
    { question: 'Может ли тревога быть без причины у здорового человека?', answer: 'У здорового человека эпизодическая тревога бывает в стрессовых ситуациях. Если тревога присутствует постоянно и без видимой причины — это сигнал, что нервная система «застряла» в режиме тревоги. Это не стыдно и лечится без таблеток.' },
    { question: 'Как отличить тревогу от интуиции?', answer: 'Интуиция — спокойное знание. Тревога — телесное напряжение, учащённое сердцебиение, навязчивые мысли. Интуиция не вызывает паники. Если вы не можете отличить — это повод разобраться с тревогой, чтобы услышать настоящую интуицию.' },
    { question: 'Что происходит в мозге при беспричинной тревоге?', answer: 'Миндалевидное тело (центр страха в мозге) становится гиперактивным. Оно посылает сигнал тревоги, даже когда реальной угрозы нет. Гипнотерапия мягко снижает активность этого центра и учит нервную систему реагировать адекватно.' },
  ],
  'utrennyaya-trevoga': [
    { question: 'Почему тревога усиливается по утрам?', answer: 'Утром происходит естественный скачок кортизола — гормона стресса, который помогает нам проснуться. Если нервная система уже истощена, этот скачок воспринимается как сигнал тревоги. Добавляется пустой желудок и ещё не включившиеся механизмы самоконтроля.' },
    { question: 'Как убрать тревогу сразу после пробуждения?', answer: 'Я учу клиентов простым дыхательным техникам, которые снижают кортизол за 3–5 минут после пробуждения. Главное — не хвататься за телефон, а дать себе 5 минут на «настройку» нервной системы. На сессиях мы закрепляем этот навык на уровне подсознания.' },
    { question: 'Кортизол по утрам — норма или патология?', answer: 'Кортизоловый скачок — физиологическая норма, но его сила и восприятие зависят от состояния нервной системы. Если утренний подъём кортизола вызывает тревогу, а не бодрость — значит, уровень стрессовой нагрузки превысил адаптационные возможности организма.' },
    { question: 'Что делать, если каждое утро начинается с тревоги и сердцебиения?', answer: 'Это классический симптом хронического стресса и тревожного расстройства. Гипнотерапия помогает снизить базовый уровень тревоги, и утро перестаёт быть временем страха. На первой сессии мы прорабатываем вечерние ритуалы и утреннее пробуждение.' },
    { question: 'Как перестать просыпаться с чувством страха?', answer: 'Страх по утрам — это часто страх самого дня, а не конкретных событий. В гипнотерапии мы работаем с глубинным ощущением безопасности, и утро постепенно становится временем спокойствия, а не страха.' },
  ],
  'trevoga-pered-snom': [
    { question: 'Почему тревога усиливается перед сном?', answer: 'Вечером снижается уровень внешней стимуляции, и внимание переключается на внутренний мир. Подсознание «выпускает» всё, что было подавлено за день. Отсутствие отвлекающих факторов и усталость делают тревожные мысли особенно интенсивными.' },
    { question: 'Как заснуть, если тревога не даёт?', answer: 'Попытки «заставить себя уснуть» только усиливают тревогу. В гипнотерапии я даю клиентам специальные аудио-программы для сна, которые мягко погружают в трансовое состояние и отключают внутренний диалог. Результат — засыпание за 5–10 минут без усилий.' },
    { question: 'Почему мысли лезут в голову, когда ложусь спать?', answer: 'Вечером мозг начинает обрабатывать информацию дня и искать решения. В норме этот процесс проходит незаметно. При тревожности он превращается в бесконечный цикл «а что если?». Гипнотерапия учит мозг отключать этот цикл по команде.' },
    { question: 'Как успокоить нервную систему перед сном?', answer: 'Нервная система не переключается со стресса на покой мгновенно — нужен «переходный мостик». На сессиях мы формируем вечерний ритуал, который я настраиваю индивидуально под ваш тип нервной системы. Это работает на уровне рефлекса.' },
    { question: 'Бессонница от тревоги — что делать?', answer: 'Бессонница от тревоги лечится через работу с причиной тревоги, а не через снотворное. Когда уровень тревоги снижается, сон восстанавливается естественным образом. Гипнотерапия даёт результат уже после первых сессий.' },
  ],
  'trevoga-posle-stressa': [
    { question: 'Почему тревога не проходит после того, как стресс закончился?', answer: 'Стрессовый опыт закрепляется в подсознании как «запись опасности». Нервная система продолжает работать в режиме угрозы, даже когда ситуация разрешилась. Нужно помочь подсознанию «перезаписать» этот опыт — этим и занимается гипнотерапия.' },
    { question: 'Сколько длится тревога после стресса?', answer: 'В норме — несколько дней до недели. Если тревога сохраняется дольше 2–3 недель — это сигнал, что нервная система не справилась с переработкой стресса и «застряла». Чем раньше начать работу, тем быстрее восстановление.' },
    { question: 'Как восстановить нервную систему после стресса?', answer: 'Восстановление требует времени и правильных инструментов. Гипнотерапия помогает нервной системе «перезагрузиться»: мы работаем с телом, дыханием и подсознанием, чтобы выйти из режима боеготовности и вернуть способность расслабляться.' },
    { question: 'Тело не расслабляется даже после отдыха — что делать?', answer: 'Это значит, что стресс «застрял» в теле на уровне мышечной памяти. Тело продолжает держать защитное напряжение, даже когда ум говорит, что всё в порядке. В гипнотерапии мы снимаем этот блок через работу с подсознанием.' },
    { question: 'Нормально ли чувствовать тревогу ещё долго после сильного стресса?', answer: 'Это естественная реакция истощённой нервной системы, но это не значит, что так должно быть. Организм сигналит, что ему нужна помощь. Гипнотерапия даёт инструменты, чтобы завершить стрессовый цикл и вернуть равновесие.' },
  ],
  'vnutrenneye-napryazheniye': [
    { question: 'Как снять внутреннее напряжение, если ничего не помогает?', answer: 'Если массажи, ванны и отдых не дают результата — значит, напряжение не мышечное, а психологическое. Оно держится на подсознательном уровне. Гипнотерапия работает именно с этим уровнем, отпуская то, что сознание удерживать не может.' },
    { question: 'Почему я постоянно напряжён, даже когда ничего не происходит?', answer: 'Хроническое напряжение — это привычка нервной системы быть в готовности. Она сформировалась как защитный механизм, но теперь работает против вас. В гипнозе мы «переучиваем» нервную систему отпускать напряжение.' },
    { question: 'Чем опасно хроническое внутреннее напряжение?', answer: 'Оно истощает нервную систему, нарушает сон, пищеварение и кровообращение, приводит к психосоматическим заболеваниям — от головных болей до проблем с сердцем. Постоянное напряжение — как работающий двигатель без выключения.' },
    { question: 'Как расслабиться, когда тело не отпускает?', answer: 'Попытки расслабиться через «надо расслабиться» — это дополнительное напряжение. Я даю клиентам техники, которые обходят сознательное усилие и работают напрямую с подсознанием. Результат — глубокое расслабление без борьбы.' },
    { question: 'Какие техники реально помогают снять внутреннее напряжение?', answer: 'Дыхательные техники, прогрессивная релаксация и гипнотерапия — три самых эффективных инструмента. Но без работы с причиной напряжения эффект временный. В гипнотерапии мы убираем и причину, и симптом.' },
  ],
  'navyazchivye-mysli': [
    { question: 'Как избавиться от навязчивых мыслей?', answer: 'С ними нельзя бороться — борьба только усиливает их. Нужно понять, откуда они берутся. Навязчивые мысли — это сигнал подсознания, что есть неразрешённое напряжение. В гипнотерапии мы убираем причину, и мысли уходят сами.' },
    { question: 'Почему я не могу перестать думать об одном и том же?', answer: 'Мозг зацикливается, потому что подсознание пытается решить проблему, но не может найти решение. Это как заезженная пластинка. Гипнотерапия помогает подсознанию завершить этот цикл и освободить внимание для настоящей жизни.' },
    { question: 'Как остановить поток мыслей в голове?', answer: 'Остановить усилием воли не получится — это только усилит поток. Нужно переключить мозг в другой режим работы. Трансовое состояние, которое я использую на сессиях, естественным образом останавливает внутренний диалог. Вы получаете тишину в голове без борьбы.' },
    { question: 'Навязчивые мысли — это психическое заболевание?', answer: 'Не всегда. Если вы осознаёте, что мысли навязчивые, и они вызывают дискомфорт — это тревожное расстройство, а не психическое заболевание. Оно прекрасно корректируется гипнотерапией. Беспокоиться стоит, если вы перестаёте отличать их от реальности.' },
    { question: 'Помогает ли гипноз при навязчивых мыслях?', answer: 'Да, и очень эффективно. Гипноз позволяет обратиться к подсознанию и «переписать» те программы, которые генерируют навязчивые мысли. Многие клиенты замечают, что после первой сессии поток мыслей становится тише и спокойнее.' },
  ],
  'strakh-budushchego': [
    { question: 'Как перестать бояться будущего?', answer: 'Страх будущего — это страх неопределённости. Невозможно гарантировать будущее, но можно вернуть опору в настоящем. В гипнотерапии мы работаем с ощущением безопасности внутри вас, чтобы будущее перестало быть угрозой.' },
    { question: 'Почему я боюсь того, что ещё не случилось?', answer: 'Мозг постоянно моделирует будущее, чтобы подготовиться к угрозам. Тревожный мозг делает это слишком активно — он рисует худшие сценарии и запускает страх. Это защитный механизм, который вышел из-под контроля.' },
    { question: 'Как жить в неопределённости и не тревожиться?', answer: 'Неопределённость — это норма жизни. Тревога начинается, когда нет внутренней опоры. На сессиях мы формируем эту опору: чувство «я справлюсь, что бы ни случилось». Когда оно есть внутри, неопределённость перестаёт пугать.' },
    { question: 'Страх будущего парализует и мешает действовать — что делать?', answer: 'Это классический замкнутый круг: страх мешает действовать, бездействие усиливает страх. В гипнотерапии мы разрываем этот круг, работая с первопричиной страха. Затем появляется энергия для действий.' },
    { question: 'Как перестать катастрофизировать и рисовать худшие сценарии?', answer: 'Катастрофизация — это привычка мозга, от которой можно отучиться. В гипнозе мы перепрограммируем реакцию на неопределённость: вместо «всё будет плохо» формируется спокойное «я справлюсь с любым развитием событий».' },
  ],
}

/* ── Default FAQs for services without specific FAQS ── */
...
/* ── Note: new anxiety-specific pages (kak-izbavitsya..., postoyannaya..., utrennyaya..., trevoga-pered-snom, trevoga-posle-stressa, vnutrenneye-napryazheniye, navyazchivye-mysli, strakh-budushchego) have their own FAQS above ── */
