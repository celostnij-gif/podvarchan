...
* **2026-07-24 Blueprint visibility fix:** Blueprint was hidden — `SeedBlueprintButton` buried in "Параметри / SEO" sub-tab, `BlueprintMissingBanner` only shown when `sections.length === 0`. Fixed by moving Blueprint management to main `HomeStudio` view as always-visible `BlueprintSection`. Verified on production: Blueprint visible + button works. SEO regression 50/50 GREEN. Builds public + admin green. Commit: `fix(home-studio): make Blueprint feature always visible in main view`.
