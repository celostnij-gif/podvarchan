...
> Активна розробка на гілці `master`. Всі фази — в `temp/admin-panel-plan.md`, трекер — `temp/admin-progress.md`.
...
### 9.6. ASK BEFORE NEXT PHASE

Перед початком НОВОЇ фази (не задачі всередині фази, а саме перехід до наступного номера Phase) — **обов'язково запитати користувача** і отримати підтвердження.

Формат: "Phase N завершена. Отримані зміни: [короткий список]. **Дозволяєте перейти до Phase N+1 — [назва]?**"

Не питати:
- Якщо фаза складається з кількох задач — продовжувати без пауз
- Якщо користувач уже дав загальну згоду (наприклад, "продовжуй по плану") — діяти в межах цієї згоди
- Для дрібних виправлень всередині поточної фази

**Порушення:** агент вважається таким, що не дотримується контракту.

### 9.7. DEPLOY VERIFICATION

Після завершення кожної фази:
1. `git add .` && `git commit -m "feat(admin): Phase N — ..."`
2. `git push` — тригер GitHub Actions деплою
3. Дочекатись завершення деплою (перевірити на GitHub: Actions → останній run → зелений)
4. **Переконатись на продакшені що сайт не деградував:**
   - curl перевірка 5+ ключових публічних сторінок (200 OK)
   - Регресійні тести: 308 редирект, 410 для .html, 404 мета-теги
   - Якщо змінювалась middleware — перевірити /admin/login доступний
5. Тільки після успішної перевірки — переходити до наступної фази
