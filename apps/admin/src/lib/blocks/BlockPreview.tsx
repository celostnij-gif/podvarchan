...
export function BlockPreview({ sectionType, content, locale }: BlockPreviewProps) {
  ...
  return (
    <div className="block-preview bg-white text-gray-900 rounded-lg overflow-hidden shadow-sm">
      {/* Preview toolbar */}
      <div className="flex items-center gap-2 border-b border-gray-200 bg-gray-50 px-3 py-1.5">
        <span className="text-xs text-gray-400 font-mono">{sectionType}</span>
        <span className="text-xs text-gray-300">•</span>
        <span className="text-xs text-gray-400">{def?.label ?? sectionType}</span>
        <div className="flex-1" />
        <span className="rounded bg-gray-200 px-1.5 py-0.5 text-[10px] text-gray-500 font-mono">
          {locale === 'ru' ? 'RU' : 'UK'}
        </span>
      </div>
      ...
      {(preview as React.ReactElement | null) && (
        <div className="border-t border-gray-100 px-3 py-1 flex items-center gap-3 text-[10px] text-gray-400">
...
function Title({ children }: { children: React.ReactNode }) {
  ...
  return <h3 className="text-lg font-bold text-gray-900 mb-2 leading-tight">{children}</h3>
}
...
function CTAButton({ text, link }: { text: string; link?: string }) {
  ...
  return (
    <div className="mt-3">
      ...
      {link && <span className="ml-2 text-xs text-gray-400">{link}</span>}
    </div>
  )
}
...
function PlaceholderImage({ alt, className = 'h-24' }: { alt?: string; className?: string }) {
  return (
    <div className={`rounded-lg bg-gray-200 flex items-center justify-center text-gray-400 text-xs ${className}`}>
      <span>🖼 {alt || 'Image'}</span>
    </div>
  )
}
...
function HeroPreview({ content, locale }: { content: Record<string, unknown>; locale: string }) {
  ...
  return (
    <PreviewContainer className="bg-gradient-to-br from-gray-900 to-gray-800 text-white p-6 text-center">
      ...
      {subtitle && <p className="text-sm text-gray-300 mb-4 max-w-md mx-auto">{subtitle}</p>}
      {cta && (
...
function TextBlockPreview({ content }: { content: Record<string, unknown> }) {
  ...
  return (
    <PreviewContainer>
      ...
      {body && (
        <div
          className="text-sm text-gray-700 leading-relaxed [&_p]:mb-2 [&_h2]:text-lg [&_h2]:font-bold [&_h2]:mt-4 [&_h2]:mb-2"
...
function StatsPreview({ content, locale }: { content: Record<string, unknown>; locale: string }) {
  ...
  return (
    <PreviewContainer>
      {title && <h3 className="text-lg font-bold text-gray-900 mb-3 text-center">{title}</h3>}
      {items.length > 0 && (
        <div className="grid grid-cols-2 gap-3">
          {items.map((item, i) => (
            <div key={i} className="text-center p-3 rounded-lg bg-gray-50 border border-gray-200">
...
function CTAPreview({ content, locale }: { content: Record<string, unknown>; locale: string }) {
  ...
  return (
    <PreviewContainer className="bg-gradient-to-r from-amber-50 to-orange-50 p-5 text-center">
      {title && <h3 className="text-xl font-bold text-gray-900 mb-2">{title}</h3>}
          ...
          {items.map((item, i) => (
            <div key={i} className="flex gap-3 pb-3 relative">
              ...
              <div className="flex flex-col items-center">
                ...
                {i < items.length - 1 && <div className="w-0.5 flex-1 bg-gray-200 -mt-0.5" />}
              </div>
              {/* Content */}
              <div className="flex-1 pb-2">
                {item.year && <span className="text-xs font-bold text-amber-600">{item.year}</span>}
                {item.title && <h4 className="text-sm font-semibold text-gray-900">{item.title}</h4>}
    ...
    <PreviewContainer>
      ...
      {images.length > 0 ? (
        <div className="grid grid-cols-3 gap-1.5">
          ...
          {images.length > 6 && (
            <div className="flex items-center justify-center h-16 rounded bg-gray-100 text-xs text-gray-500">
              +{images.length - 6}
            </div>
          )}
        </div>
  ...
  return (
    <PreviewContainer>
      ...
      {youtubeId ? (
        ...
        <div className="rounded-lg bg-gray-100 p-3 text-center text-xs text-gray-400">
    ...
    <PreviewContainer>
      ...
      <div className="grid grid-cols-2 gap-2">
        {Array.from({ length: Math.min(4, maxItems) }).map((_, i) => (
          <div key={i} className="rounded-lg border border-gray-200 p-2.5 bg-gray-50">
  ...
  return (
    <PreviewContainer>
      ...
      <div className="mt-2 flex items-center gap-1 text-[10px] text-gray-400">
        <span className="rounded bg-gray-100 px-1.5 py-0.5">{group}</span>
...
function ContactFormPreview({ content, locale }: { content: Record<string, unknown>; locale: string }) {
  ...
  return (
    <PreviewContainer className="bg-gray-50 p-4">
      {title && <h3 className="text-lg font-bold text-gray-900 mb-1">{title}</h3>}
      {subtitle && <p className="text-sm text-gray-600 mb-3">{subtitle}</p>}
      <div className="space-y-2">
        <div className="h-8 rounded-lg border border-gray-300 bg-white px-3 flex items-center text-xs text-gray-400">
          {locale === 'uk' ? 'Ваше ім\'я' : 'Ваше имя'}
        </div>
        <div className="h-8 rounded-lg border border-gray-300 bg-white px-3 flex items-center text-xs text-gray-400">
          {locale === 'uk' ? 'Email або телефон' : 'Email или телефон'}
        </div>
        <div className="h-16 rounded-lg border border-gray-300 bg-white px-3 py-2 text-xs text-gray-400">
    ...
    <div className="space-y-1.5">
      {keys.map((key) => (
        <div key={key} className="flex gap-2 text-xs">
          <span className="font-mono text-gray-500 shrink-0">{key}:</span>
          <span className="text-gray-700 truncate">{str(content[key])}</span>
        </div>
      ))}
    </div>
  )
