...
export async function updateInternalNote(leadId: string, formData: FormData) {
  ...
  const lead = await db.select({ internalNote: contactLeads.internalNote }).from(contactLeads).where(eq(contactLeads.id, leadId)).get()
  if (!lead) throw new Error('Заявку не знайдено')

  await db.update(contactLeads).set({ internalNote: parsed.data.note, updatedAt: await now() }).where(eq(contactLeads.id, leadId))
  await writeAuditLog({ userId: user.id, action: 'UPDATE', entityType: 'LEAD', entityId: leadId, before: { internalNote: lead.internalNote }, after: { internalNote: parsed.data.note } })
