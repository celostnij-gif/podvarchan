  ...
  return (
    <div>
      <div className="mb-6 flex items-center justify-between">
        <h1 className="text-2xl font-bold text-zinc-100">Користувачі</h1>
      </div>
        ...
        <table className="min-w-full divide-y divide-zinc-800">
          <thead className="bg-zinc-800/30">
            <tr>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-zinc-500">Імʼя</th>
              ...
              <th className="px-4 py-3 text-left text-xs font-medium uppercase tracking-wider text-zinc-500">Останній вхід</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-zinc-800">
            {allUsers.length === 0 ? (
              <tr><td colSpan={5} className="px-4 py-8 text-center text-sm text-zinc-500">Користувачів ще немає.</td></tr>
            ) : allUsers.map((user) => (
...
function RoleBadge({ role }: { role: string }) {
  ...
  const labels: Record<string, string> = {
    OWNER: 'Власник', ADMIN: 'Адмін', EDITOR: 'Редактор', VIEWER: 'Спостерігач', USER: 'Користувач',
  }
...
function ActiveBadge({ active }: { active: boolean }) {
  return active
    ? <span className="inline-block rounded-full bg-green-900/30 px-2.5 py-0.5 text-xs font-medium text-green-400 border border-green-700/30">Активний</span>
    : <span className="inline-block rounded-full bg-red-900/30 px-2.5 py-0.5 text-xs font-medium text-red-400 border border-red-700/30">Неактивний</span>
}
