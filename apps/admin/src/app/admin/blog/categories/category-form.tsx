    ...
    <form action={formAction} className="space-y-6">
      ...
      <fieldset className="rounded-lg border p-4">
        ...
        <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
          <div>
            <label htmlFor="slugBase" className="block text-sm font-medium text-zinc-300">Slug Base *</label>
...
function LocaleSection({ label, locale, tr }: {
  ...
  return (
    <fieldset className="rounded-lg border border-zinc-700/50 p-4">
      <legend className="text-sm font-semibold text-amber-400">{label} — переклад</legend>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
        <div>
          <label htmlFor={`${locale}_slug`} className="block text-sm font-medium text-zinc-300">Slug *</label>
          <input id={`${locale}_slug`} name={`${locale}_slug`} defaultValue={tr(locale, 'slug')} required
            className="mt-1 block w-full rounded-lg border border-zinc-700 bg-zinc-800/50 px-3 py-2 text-sm text-zinc-200 placeholder-zinc-500 focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/30" />
        </div>
        <div>
          <label htmlFor={`${locale}_name`} className="block text-sm font-medium text-zinc-300">Назва *</label>
          <input id={`${locale}_name`} name={`${locale}_name`} defaultValue={tr(locale, 'name')} required
            className="mt-1 block w-full rounded-lg border border-zinc-700 bg-zinc-800/50 px-3 py-2 text-sm text-zinc-200 placeholder-zinc-500 focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/30" />
        </div>
        <div className="sm:col-span-2">
          <label htmlFor={`${locale}_description`} className="block text-sm font-medium text-zinc-300">Опис</label>
