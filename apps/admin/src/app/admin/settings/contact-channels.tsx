  ...
  function renderForm(channel?: Channel) {
    return (
      <form onSubmit={handleSubmit} className="grid grid-cols-6 gap-2 items-end">
        ...
        <select
          name="type"
          defaultValue={channel?.type ?? 'CUSTOM'}
          className="rounded-lg border border-zinc-700 bg-zinc-800/50 px-2 py-1.5 text-sm text-zinc-200 focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/30"
          required
        >
          {TYPES.map((t) => (
            <option key={t} value={t}>
              {CHANNEL_TYPE_LABELS[t] ?? t}
            </option>
          ))}
        </select>
        <input
          ...
          className="rounded-lg border border-zinc-700 bg-zinc-800/50 px-2 py-1.5 text-sm text-zinc-200 placeholder-zinc-500 focus:border-amber-500/50 focus:outline-none focus:ring-1 focus:ring-amber-500/30"
          placeholder="Мітка"
        />
        <input
          name="value"
        ...
        <div className="flex gap-2 items-center">
          <label className="text-xs text-zinc-400">
            <input
              type="checkbox"
              name="isPrimary"
              defaultChecked={channel?.isPrimary ?? false}
          ...
          <label className="text-xs text-zinc-400">
            <input
              type="checkbox"
              name="isEnabled"
              defaultChecked={channel?.isEnabled ?? true}
              className="rounded border-zinc-700 bg-zinc-800 text-amber-500 focus:ring-amber-500"
            />{' '}
            Увімкнено
          </label>
    ...
    <div className="space-y-2">
      {channels.map((ch) =>
        editing === ch.id ? (
          ...
          <div
            key={ch.id}
            className="flex items-center gap-3 p-2.5 border border-zinc-700/50 rounded-lg bg-zinc-900/30"
          >
            <span
            ...
            {ch.isPrimary && (
              <span className="text-xs bg-amber-900/30 text-amber-400 border border-amber-700/30 px-1.5 py-0.5 rounded font-medium">
                PRIMARY
              </span>
            )}
            <button
              onClick={() => setEditing(ch.id)}
              className="rounded px-2 py-1 text-xs text-amber-400 hover:bg-zinc-800 hover:text-amber-300 transition-colors"
            >
              Редагувати
            </button>
            <button
